...
window.setRange = function(range) {
  ...
  if (!tracker) return;
  // Filter classification changes by time range
  const result = JSON.parse(tracker.changes_since('', rangeToDate(range)));
  const el = document.getElementById('vus-list');
  el.innerHTML = `<div style="color:#64748b;font-size:11px;margin-bottom:6px;">
    ${formatNumber(result.total)} classification changes in ${rangeLabel(range)}
  </div>`;
  (result.changes || []).forEach(c => {
    el.innerHTML += `<div class="row">
      <span class="gene">${esc(c.gene)}</span>
      <span class="hgvs">${esc((c.hgvs||'').substring(0,25))}</span>
      <span class="val">${shortClass(c.old)}→${shortClass(c.new)}</span>
    </div>`;
  });
};
