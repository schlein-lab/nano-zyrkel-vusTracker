...
window.exportCSV = function() {
  ...
  csv += `Total Variants in ClinVar,${indexCache.total_variants}\n`;
  csv += `Classification Changes,${indexCache.total_reclassifications}\n`;
  csv += `Date Range,"${indexCache.date_range?.from} to ${indexCache.date_range?.to}"\n\n`;
  if (g) {
    csv += `\nGene Focus: ${gene}\n`;
    csv += `Total,${g.total}\n`;
    csv += `Pathogenic,${g.pathogenic}\n`;
    csv += `Likely Pathogenic,${g.likely_pathogenic}\n`;
    csv += `VUS,${g.vus}\n`;
    csv += `Likely Benign,${g.likely_benign}\n`;
    csv += `Benign,${g.benign}\n`;
    csv += `Conflicting,${g.conflicting}\n`;
  }
  csv += '\nTop Genes\nGene,Variants\n';
  (indexCache.top_genes || []).forEach(([name, count]) => { csv += `${name},${count}\n`; });
  csv += `\nGenerated by nano-zyrkel-vusTracker\nData: NCBI ClinVar (public domain)\n`;

  const blob = new Blob([csv], {type: 'text/csv'});
  const a = document.createElement('a');
  a.href = URL.createObjectURL(blob);
  a.download = `vusTracker_${gene}_${new Date().toISOString().slice(0,10)}.csv`;
  a.click();
};
